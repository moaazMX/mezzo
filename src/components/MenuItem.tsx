...
export default function MenuItem({ item, onAddToCart }: MenuItemProps) {
  ...
  return (
    <>
      <div
        ref={itemRef}
        className="mx-menu-item group/menu-item relative flex flex-col overflow-hidden rounded-2xl border border-primary/10 bg-[hsl(var(--color-surface))] shadow-md transition-[box-shadow,border-color,transform] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 motion-reduce:transition-none motion-reduce:hover:translate-y-0 [@media(hover:none)]:hover:translate-y-0 [@media(hover:hover)]:h-full"
      >
        ...
        >
          <div className={`mx-menu-hero relative w-full overflow-hidden bg-black/30 sm:min-h-[13rem] md:min-h-[14rem] [@media(max-width:768px)_and_(pointer:coarse)]:aspect-[5/4] [@media(hover:hover)]:aspect-auto [@media(hover:hover)]:flex-1 [@media(hover:hover)]:min-h-0 ${display.squareImages ? 'aspect-square min-h-0' : 'aspect-[5/4] min-h-[11.5rem] md:aspect-[4/3]'} ${display.hideImage ? 'hidden' : ''}`}>
          ...
          {!display.hideItemName && (
          <div className="mx-menu-meta space-y-1.5 border-t border-primary/25 bg-[hsl(var(--color-surface))] px-3 py-2.5 text-right sm:px-3.5 sm:py-3 [@media(hover:hover)]:bg-[hsl(var(--color-surface))] [@media(hover:hover)]:border-t-0 [@media(hover:hover)]:pb-14">
            <h3 className="desktop-force-white line-clamp-2 text-sm font-black leading-tight text-white sm:text-base">
              {title}
            </h3>
            {subtitle && subtitle !== title && (
              <p className="desktop-force-white-muted line-clamp-1 text-[10px] text-white/70 sm:text-xs">{subtitle}</p>
            )}
            {!display.hideDescription && description && (
              <p className="desktop-force-white-muted hidden text-[10px] leading-snug text-white/80 sm:text-[11px] [@media(hover:hover)]:line-clamp-1 [@media(hover:hover)]:block">
                {description}
              </p>
            )}
          </div>
        ...
        <div className={`flex items-end justify-between gap-2 bg-[hsl(var(--color-surface))] px-3 pb-2.5 pt-1 sm:px-3.5 sm:pb-3 [@media(hover:hover)]:absolute [@media(hover:hover)]:bottom-0 [@media(hover:hover)]:inset-x-0 [@media(hover:hover)]:bg-transparent [@media(hover:hover)]:border-t-0 [@media(hover:hover)]:z-20 ${display.hidePrice ? 'hidden' : ''}`}>
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {showDetails && createPortal(
        <div
          className="menu-item-modal-backdrop fixed inset-0 z-[150] flex justify-center bg-black/88 backdrop-blur-[3px] [@media(hover:none)]:items-stretch [@media(hover:hover)]:items-center [@media(hover:hover)]:p-5 [@media(hover:hover)]:lg:p-8"
          onClick={() => setShowDetails(false)}
        >
          <div
            ref={modalRef}
            className="menu-item-modal-panel relative flex max-h-[100dvh] w-full max-w-full flex-col overflow-hidden bg-[#0c0816] shadow-2xl [@media(hover:hover)]:max-h-[min(94vh,920px)] [@media(hover:hover)]:max-w-[min(100%,42rem)] [@media(hover:hover)]:lg:max-w-[min(100%,52rem)] [@media(hover:hover)]:rounded-[1.85rem] [@media(hover:hover)]:border [@media(hover:hover)]:border-white/12"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setShowDetails(false)}
              className="menu-item-modal-close absolute start-3 top-3 z-30 flex h-11 w-11 items-center justify-center rounded-full border-2 border-white/85 bg-[rgba(15,23,42,0.82)] text-white backdrop-blur-sm transition-colors hover:bg-[rgba(15,23,42,0.92)]"
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-y-auto custom-scrollbar">
              <div className="menu-item-modal-hero relative w-full shrink-0 bg-black">
                ...
                <div className="menu-item-modal-hero-gradient pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/45 to-transparent [@media(hover:hover)]:h-28" />
              </div>
              ...
              <div className="space-y-4 px-4 pb-10 pt-4 text-right [@media(hover:hover)]:px-8 [@media(hover:hover)]:pb-8 [@media(hover:hover)]:pt-6">
                ...
                <div className="menu-item-modal-footer flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
